...
---
A signed Android APK for teachers exists at `https://expressonly.in/school/downloads/kis-attendance.apk` and is also linked from the web Login page. Source lives in `/Users/manjeetsaini/Documents/GitHub/school-management/android/` (Kotlin + Jetpack Compose, ~3 MB APK, signed v2/v3, minSdk 26 / targetSdk 34, package `com.expressonly.kisattendance`). Talks to the existing FastAPI backend at `expressonly.in/school/api/` — no backend changes; class scope enforced server-side via the JWT's `allowed_classes`.
